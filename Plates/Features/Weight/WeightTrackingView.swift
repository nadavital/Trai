        ...
        NavigationStack {
            ScrollView {
                VStack(spacing: 20) {
                    ...
                    // Weight history list
                    WeightHistoryList(entries: Array(weightEntries.prefix(10)))
                }
                .padding()
            }
...
struct CurrentWeightCard: View {
    ...
    private var progressToGoal: Double? {
        ...
        // This is simplified - would need starting weight for accurate progress
        return nil
    }
    ...
    var body: some View {
        VStack(spacing: 12) {
            ...
            if let target = targetWeight {
                let difference = entry.weightKg - target
                HStack {
                    Image(systemName: difference > 0 ? "arrow.down" : "arrow.up")
                    Text("\(abs(difference), format: .number.precision(.fractionLength(1))) kg to goal")
                }
                .font(.subheadline)
